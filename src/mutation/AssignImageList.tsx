...
export default function AssignImageList(props: { hideTitle?: boolean }) {
  ...
  if (error) {
    return (
      <ResourceListError
        ...
        sectionTitle={props.hideTitle ? undefined : 'AssignImage Mutation'}
      />
    );
  }

  if (!items) {
    return <Typography>Loading AssignImage Mutation...</Typography>;
  }

  return (
    <SectionBox title={props.hideTitle ? undefined : 'AssignImage Mutation'}>
        ...
        columns={[
          {
            label: 'Name',
            getter: item => (
              <HeadlampLink routeName={RouteName.AssignImage} params={{ name: item.metadata.name }}>
                {item.metadata.name}
              </HeadlampLink>
            ),
          },
          ...
          },
          {
            label: 'Assign Value',
            getter: item => {
              const params = item.jsonData?.spec?.parameters || {};
              const parts = [];
              if (params.assignDomain) parts.push(`Domain: ${params.assignDomain}`);
              if (params.assignPath) parts.push(`Path: ${params.assignPath}`);
              if (params.assignTag) parts.push(`Tag: ${params.assignTag}`);
